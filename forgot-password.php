...
<head>
<meta charset="utf-8">
<title>Self Aware Stories :: Forgot Password</title>
...
<script type="text/javascript">
			...
			if(val==''){
				...
				$.ajax({
					...
					if(response==''){
					   ...
					   $("#error-field").html("Your login details has been sent to your registered Email Id").css('color','green')
                    }
					else if(response=='error'){
                       $("#error-field").html("Please enter your registered Email Id").css('color','red')
                    }					
    ...
    function validateEmail(fld){  
		...
		var error=""; 
		var emailFilter =  /^[0-9a-zA-Z]+([0-9a-zA-Z]*[._+])*[0-9a-zA-Z]+@[0-9a-zA-Z]+([-.][0-9a-zA-Z]+)*([0-9a-zA-Z]*[.])[a-zA-Z]{2,6}$/ ;
		...
		if(fld.value == "" || fld.value == null){		   
           ...
           $("#error-field").html("Enter a valid Email Id").css('color','red');		   		   
		}
		else{
	...
	<div id="wrapp">
		...
		<div id="contentlogin">
			...
			<div class="container"> <br><br><br>
				...
				<h4 class="widget-title" style=" background: #FFF;">Forgot Password</h4>
				<form id="frm1" name="frm1" action=""  class="simple-form" method="post" onsubmit="return validateForm()">
					<div id="error-field" ></div>
					<input type="hidden" name="formerror" id="formerror">
                    <fieldset>
						<i class="icon-user tooltip left" title="Your Email"></i>
						<input id="email"  placeholder="Your Email"  type="text"  name="email"/>
					</fieldset>
										
					<div class="one-fourthlogin">
						<input type="submit" value="Submit" name="sub" class="button big grey round"/>
                    </div>
                    <div class="one-fourthlogin">
