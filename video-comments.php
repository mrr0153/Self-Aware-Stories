...
$(document).ready(function(){
	...
	function loadData(page){
        ...
        $.post('getforumcomments.php',{"page":1,'cid':cid,'vid':vid},function(data){
	...
	$('#themetitles').change(function(){
	   		
		var cid=$(this).val()
		window.location.href='video-comments.php?cid='+cid;
		
	});	
	
	
	$('#videotitles').change(function(){
		...
		<div id="content"><br><br>
	      ...
	      <div class="one">
	       ...
	       <div class="vcommentlist">
		    <h4 style="color: #005292;  padding-left: 0px;margin-bottom: 10px;">Select Theme</h4>		    
			<!-- 	
			<?php 
			...
			?>
			 <select id="themetitles" name="themetitles">
			<?php
			...
			<select id="themetitles" name="themetitles">
			  ...
			  <?php
				...
				while($row = mysql_fetch_array($result)) {
					...
					$vtitles.=stripslashes($row['cat_name']). "||||";	
			  ?>
			    <option value="<?php echo $row['cat_id']; ?>" <?php if(isset($_REQUEST['cid']) && $_REQUEST['cid'] == $row['cat_id'] ){ echo "selected"; }?>><?php echo stripslashes($row['cat_name']); ?></option>
			  <?php } ?>
			</select>			
		    <br>
            <h4 style="color: #005292;  padding-left: 0px;margin-bottom: 10px;">Select Video</h4>				
			<select id="videotitles" name="videotitles">
			  <option value="">Select Video</option>
              
			  <?php
			    $sql="SELECT * FROM sas_videos WHERE cat_id = '$_REQUEST[cid]' ";
				$result = mysql_query($sql) ;			
				$i=0;
				while($row = mysql_fetch_array($result)) {										
			  ?>
			    <option value="<?php echo $row['id']; ?>" <?php if($i==0){ echo 'selected';} ?> ><?php echo stripslashes($row['prod_name']); ?></option>
			  <?php $i++; } ?>
		   ...
		   <div class="vcomments" >
            <div class="vcomments-block">
             <div class="comment-content">	
			   ...
			   <div id="loading" style="text-align:center;text-align: center; margin: 7% 32%; position: absolute;"> </div>
