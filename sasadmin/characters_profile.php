...
$filePath="characters_profile.php";
...
if(isset($_POST['add_edit']))
{
  extract($_POST);
	...
	{
	     $fileName ='char'.date("His").'_'.$_FILES['photopath']['name'];
		 $fileTempName=$_FILES['photopath']['tmp_name'];
		 //move the file
		 if ($s3->putObjectFile($fileTempName, "sas-characterphotos", $fileName, S3::ACL_PUBLIC_READ)) {
	...
	else
	{
	 ...
	 if(isset($act))
	  ...
	  if(manageprofilesdetails($char_name,$title,$fileName,$description,$pid,$pcid,$status,$id)) {
		
		$msg = "Program Updated Successfully";
	  }
	  
      header("location:characters_profile.php?pid=".$pid."&pcid=".$pcid);	  
 }
...
//function for delete
 ...
 {
     $sqldel = mysql_query("select * from sas_chars  WHERE id='".$_REQUEST["del"]."'");
	 ...
	 if(mysql_query($delete)){
       ...
       $s3->deleteObject("sas-characterphotos", $rowdel['photopath']);
	 }
...
function delete_record(id,pid,pcid)
{
	...
	{
		document.location.href='characters_profile.php?del='+id+'&pid='+pid+'&pcid='+pcid;
	}
}
	...
	<!--display when user lands on the page-->									
                    							...
                    							<tr>
													...
													<td width="18%" align="right">
														<table  cellpadding=1 cellspacing=2 id=toplinks>
															...
															<a href="characters_profile.php?Call=edit&pcid=<?=$pcid?>&pid=<?=$pid?>" class="blue_txt">Add&nbsp;New&nbsp;Character</a>															</td>
								    ...
								    <table width="100%" border="0" cellspacing="1" cellpadding="4" align=center  bgcolor="#EFD57E">
									 ...
									 <tr bgcolor = <?php echo $bgcolor?>>
										...
										<TD align="center" class="gray_txt"><img src="https://s3-us-west-2.amazonaws.com/sas-characterphotos/<?php echo $line['photopath'];?>" width="60" height="60"></TD>
										...
										<a href="characters_profile.php?Call=edit&pid=<?php echo $pid;?>&pcid=<?php echo $pcid;?>&id=<?php echo $line['id']?>" class="links">Edit</a>
         ...
         <select id="select2"  name="pid" class="admin_selectbox">
           <option  value="<?php if(isset($id)) echo $id;?>" <?php if(isset($id)) { ?> selected="selected" <?php }?>><?php if(isset($prod_name)){echo $prod_name;} else { echo "select";}?></option>
			...
			<tr>
				...
				<td width="70%" align="left" bgcolor="#F7F7F7"><img src="https://s3-us-west-2.amazonaws.com/sas-characterphotos/<?php if(isset($photopath)) print($photopath);?>" width="150" height="150" border="0"></td>
			</tr>
			<tr>
			  <td height="30" align="right" bgcolor="#F7F7F7"><strong>Character Profile:</strong></td>
			  <td align="left" bgcolor="#F7F7F7">
			  <textarea name="description" id="description"  ><?php if(isset($description))print($description);?></textarea>
			  </td>
			  </tr>
