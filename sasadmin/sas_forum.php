 ...
 if(isset($_POST['add_edit']))
 {
  ...
  // print_r($_POST);
   ...
   if(isset($act))
   {
	 ...
	 $upsql="UPDATE `sas_comment_replies` SET `reply` = '".addslashes(stripslashes($reply))."',`cmtid` = '$cmtid',`vid` = '$vid',`adminid`=$adminid,`replieddate`=now(),`status` = '$Status' WHERE `rid`=$_REQUEST[rid]";	   
  ...
  {
	    $inssql="INSERT INTO `sas_comment_replies` (`reply`,`cmtid`,`vid`,`adminid`,`replieddate`, `status` ) VALUES ('".addslashes($reply)."','$cmtid','$vid','$adminid',now(),'$Status')";
	...
	<!--display when user lands on the page-->									
       ...
       <table width="100%" border="0" cellspacing="1" cellpadding="4" align=center  bgcolor="#EFD57E">
         ...
         <?php         
			...
			$mddtls=mysql_query("select * from sas_moderators where admin_id=".$_SESSION['adminId']);
			...
			while($line=mysql_fetch_array($rs))
			{
			   ...
			   $rsvcats=mysql_query("select * from sas_themecategories where cat_id=".$line['cid']);
			   $rrsvcats = mysql_fetch_array($rsvcats);
			   
               $rpldtls=mysql_query("select * from sas_comment_replies where cmtid='$line[id]' and adminid=".$_SESSION['adminId']);
	      ...
	      ?>
			...
			<TD align="center" class="gray_txt"><?php echo stripslashes($rrsvcats['cat_name'])?></TD>
			...
			<?php
			  ...
			  if( $rrpldtls['adminid'] == $_SESSION['adminId']){			  
			?>				
