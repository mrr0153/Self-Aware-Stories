<?php 
 ...
 $filePath="manage_moderators.php";
 ...
 if(isset($_POST['add_edit'])){
	...
	if(isset($act)){
		...
		$upsql="UPDATE `sas_moderators` SET `username` = '$username',
		...
		if(mysql_num_rows(mysql_query("select * from sas_moderators where username='$username'"))==0){

			$inssql="INSERT INTO `sas_moderators` (`username` , `password` , `first_name` , `last_name` , `email` ,`colorcode`, `date_added` , `status` )

			VALUES ('$username','$password','$firstName','$lastName','$email','$colorcode',now(),'$status')";

			mysql_query($inssql);

			$_SESSION['sessionMsg']='Moderator created';
    ...
    header("Location: manage_moderators.php");
 ...
 if(isset($_REQUEST["del"])){
	 ...
	 $delete="delete from sas_moderators WHERE admin_id='".$_REQUEST["del"]."'";

	 $delete_query=mysql_query($delete);

	 $_SESSION['sessionMsg']='Moderator deleted Successfully';
...
//alert message for delete
 ...
 function delete_record(id){
	...
	{
		document.location.href='manage_moderators.php?del='+id;
	}
 }
  ...
  <table width="100%"  border="0" cellspacing="0" cellpadding="0">
	...
	<?php if((!isset($_REQUEST['id']))&&(!isset($_REQUEST['Call'])) ){ ?>
			...
			<table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#B1B1B1">
				...
				<table width="99%"  border="0" align="center" cellpadding="0" cellspacing="0">
					...
					<tr>
					<td width="105">
					<a href="manage_moderators.php?Call=edit" class="blue_txt">Add&nbsp;New&nbsp;Moderator</a>
					</tr>
					</table>
					</td>
					</tr>
			...
			<?php
				...
				$sql="SELECT * FROM sas_moderators limit $start,$q_limit";
            ...
            ?>
				...
				<tr bgcolor = <?=$bgcolor?>>
				  ...
				  <a href="manage_moderators.php?Call=edit&id=<?=$line['admin_id']?>" class="links">Edit</a>
	...
	<!--display when user lands on the page-->									
		...
		if(isset($_REQUEST['id'])){
			...
			$sql = "SELECT * FROM sas_moderators  where admin_id=$_REQUEST[id]";
		...
		<table width="100%" border="0" cellpadding="3" cellspacing="1" class="outercolor gray_txt">
            <tr class="header_row">
			 <td colspan="3" align="left"> Edit/Add New Moderator</td>
			</tr>
			...
			<tr>
				...
				<td width="70%" align="left" bgcolor="#F7F7F7">
					<input name="password" type="test" class="textbox" id="section_data" size="30" value="<?php if(isset($password)){  print($password);}?>"/>
				</td>
			</tr>		
			...
			<tr>
				<td width="30%" height="30" align="right" bgcolor="#F7F7F7">
					<strong> Color Code : </strong>	</td>
				<td width="70%" align="left" bgcolor="#F7F7F7">
				<input type="text" name="colorcode" value="<?php if(isset($colorcode)){ print($colorcode);}?>" >				  
				</td>			   
			</tr>
