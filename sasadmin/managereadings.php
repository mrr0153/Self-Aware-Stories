 ...
 if(isset($_POST['add_edit']))
 {
  ...
  if(isset($act))  
  {
        if(is_uploaded_file($_FILES['filepath']['tmp_name'])){
          ...
          header("Location:managereadings.php");
          ...
          header("Location:managereadings.php?start=$start");
...
//function for delete
 ...
 if(isset($_REQUEST["del"]))
 {
   ...
   if( $delete_query=mysql_query($delete)){
      ...
      $s3->deleteObject("sas-userfiles", $rowdel['rfilepath']);
	  
	  mysql_query("delete from sas_userreadings WHERE rid='".$_REQUEST["del"]."'");
...
function delete_record(rid)
{
	...
	{
		document.location.href='managereadings.php?del='+rid;
	}
}
...
function validate1()
{
	var a=document.frm_add_edit.cid.value;

	if (a == "") { 

	alert("Please Select Category Name ");

	document.frm_add_edit.cid.focus();

  	return false;
  	}
	
	var b=document.frm_add_edit.vid.value;

	if (b == "") { 

	alert("Please Select Video Name ");

	document.frm_add_edit.vid.focus();

  	return false;
  	}
	
	var c=document.frm_add_edit.rtitle.value;

	if (c == "") { 

	alert("Please Enter Reading Title ");

	document.frm_add_edit.rtitle.focus();

  	return false;
  	}
	
	var d=document.frm_add_edit.filepath.value;

	if (d == "") { 

	alert("Please Select File ");

	document.frm_add_edit.filepath.focus();
	...
	<!--display when user lands on the page-->									
                    ...
                    <tr>
						...
						<td width="18%" align="right">
							<table  cellpadding=1 cellspacing=2 id=toplinks>
                        		<tr>
                        		   <td width="50" >													
                                     <a href="managereadings.php?Call=edit" class="blue_txt">Add&nbsp;New&nbsp;Reading&nbsp;</a>															</td>
                                </tr>
                            </table>
                        </td>
                    </tr>
			...
			?>
				...
				<tr bgcolor = <?php echo $bgcolor?>>
					...
					<!-- <a href="managereadings.php?Call=edit&rid=<?php echo $line['rid']?>" class="links">Edit</a><span class="links" style="text-decoration:none">&nbsp;|</span>&nbsp; -->
