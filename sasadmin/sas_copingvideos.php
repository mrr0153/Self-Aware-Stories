<?php 
   ...
   if(isset($act)){
	...
	if(is_uploaded_file($_FILES['prod_image']['tmp_name']))
	{
		 ...
		 if ($s3->putObjectFile($fileTempName, "sas-productimages", $fileName, S3::ACL_PUBLIC_READ)) {
			...
			$_SESSION['sessionMsg']='Coping Video Updated Successfully';
    ...
    { 
	  ...
	  if($norows=mysql_num_rows($selcat)<=0)
	  {
		 ...
		 if ($s3->putObjectFile($fileTempName, "sas-productimages", $fileName, S3::ACL_PUBLIC_READ)) {
		    ...
		    VALUES ( '$pcid','$pid','$prod_name','$prod_image','$vcode','$cvtype','$price1','".addslashes($description)."','$pagetitle','$btopics','$tags','$mresources','$mkeywords','$mdescription','$status',now(),'','$videono')";

		    mysql_query($inssql) or die(mysql_error());

		    $_SESSION['sessionMsg']='Coping Video Inserted Successfully';
	  ...
	  { 
	    $_SESSION['sessionMsg']="Course name already exits";
		header("Location:sas_copingvideos.php?pcid=$pcid&pid=$pcid&error=1");
		exit;
 ...
 if(isset($_REQUEST["del"])) {
	...
	$sqldel = mysql_query("select * from sas_copvideos WHERE id=".$_REQUEST['del']."'");
	...
	$_SESSION['sessionMsg']='Coping Video Deleted Successfully';
...
{  //alert(id,pid,pcid)
	if(confirm("Are you sure to delete this Coping Video?"))
	{
	...
	<tr>
     <td height="40" align="left" background="images/heading_bg.gif" bgcolor="#FFFFFF" class="h1"><!--this display first-->
      ...
      <!--	<td width="43%" align="center" class="gray_txt">Search By Theme Code: <input type="text" name="book_code"><input type="submit" name="codesearch" value="Search">
        ...
        <tr>
    	  <td>
		  <a href="sas_copingvideos.php?Call=edit&pcid=<?=$pcid?>&pid=<?=$pid?>" class="blue_txt">Add New Coping Video </a>
  		  </td>
        </tr>
		</table>
	  </td>
...
<table width="100%" border="0" cellspacing="1" cellpadding="4" align=center  bgcolor="#EFD57E">
  <TD width="25%" align="center"  class="header_row"><a class="whiteTxtlink">Video Name</a></TD>
	...
	<?php
     ...
     if(isset($_REQUEST['id'])){
			...
			<tr>
				...
				<td width="70%" align="left" bgcolor="#F7F7F7"><img src="https://s3-us-west-2.amazonaws.com/sas-productimages/<?php if(isset($prod_image)) { print($prod_image); }else{ '../images/portfolio/portfolio-item-1.jpg'; }?>" width="150" height="100" border="0"></td>
			</tr>
			<tr>
			 <td width="30%" height="30" align="right" bgcolor="#F7F7F7">
