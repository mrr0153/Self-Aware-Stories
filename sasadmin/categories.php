 ...
 if(isset($_POST['add_edit'])){
	...
	if(isset($act)) {
		 ...
		 if(is_uploaded_file($_FILES['res_img']['tmp_name'])){
			    ...
			    $fileName = 'theme_'.date("His").'_'.$_FILES['res_img']['name'];
			   	$fileTempName=$_FILES['res_img']['tmp_name'];		    
				//move the file
				if ($s3->putObjectFile($fileTempName, "sas-productimages", $fileName, S3::ACL_PUBLIC_READ)) {
					//echo "<strong>We successfully uploaded your file.</strong>";					
					mysql_query("UPDATE `sas_themecategories` SET `cat_name` = '$cat_name',`imagepath` = '$fileName',`vcode` = '$vcode',`sort_order` = '',`catdesctiption` = '".addslashes($catdesctiption)."',`timeline`='$timeline',date_added=now(),`status` = '$Status' WHERE `cat_id`=$_REQUEST[id]") or die(mysql_error());
				...
				$fileName = $prev_img;
				mysql_query("UPDATE `sas_themecategories` SET `cat_name` = '$cat_name',`imagepath` = '$fileName',`vcode` = '$vcode',`sort_order` = '',`catdesctiption` = '".addslashes($catdesctiption)."',`timeline`='$timeline',date_added=now(),`status` = '$Status' WHERE `cat_id`=$_REQUEST[id]");
                $_SESSION['sessionMsg']='Theme Updated successfully';
			 }			 	 
			 ...
			 exit;
			  ...
			  if ($s3->putObjectFile($fileTempName, "sas-productimages", $fileName, S3::ACL_PUBLIC_READ)) {
				  ...
				  {
					 $inssql="INSERT INTO `sas_themecategories` (`cat_name`,`imagepath`,`vcode`,`sort_order`,`catdesctiption`,`timeline`,`date_added`, `status` ) VALUES ('$cat_name','$fileName','$vcode','$sort_order','$catdesctiption','$timeline','now()','$Status')";
  ...
  if(isset($_REQUEST["del"])){
	 ...
	 $sqldel = mysql_query("select * from sas_themecategories  WHERE cat_id='".$_REQUEST["del"]."'");
	 ...
	 if(mysql_query($delete)){
       ...
       $s3->deleteObject("sas-productimages", $rowdel['imagepath']);
	 }
		 ...
		 <?php
		  ...
		  ?>
			 ...
			 <tr bgcolor = <?php echo $bgcolor;?>>
				...
				<TD align="center" class="gray_txt"><img src="https://s3-us-west-2.amazonaws.com/sas-productimages/<?php echo $line['imagepath']?>" width="100" height="50" ></TD>
				...
				<a href="product_manager.php?pcid=<?php echo $line['cat_id']?>" class="links">Add/Edit Videos</a><span class="links" style="text-decoration:none">&nbsp;|</span>&nbsp;
	  ...
	  //display for edit or new starts here
		...
		<table width="100%" border="0" cellpadding="3" cellspacing="1" class="outercolor gray_txt">
            ...
            <tr>
				...
				<td width="70%" align="left" bgcolor="#F7F7F7">
				<input name="res_img" type="file" class="textbox" id="imagepath" size="30" value=""/>&nbsp;
				<input type="hidden" name="prev_img" value="<?php if(isset($imagepath)) print($imagepath);?>"/>
				<b><font color="#FF0000">(width 400px height 527px)</font></b></td>
			</tr>
			<tr>
				<td width="30%" height="30" align="right" bgcolor="#F7F7F7">
				<strong> Current Image <span class="featured">*</span> : </strong>	</td>
				<td width="70%" align="left" bgcolor="#F7F7F7"><img src="https://s3-us-west-2.amazonaws.com/sas-productimages/<?php if(isset($imagepath)) print($imagepath);?>" width="150" height="85" border="0"></td>
			</tr>
            <tr>
            <tr>
				<td width="30%" height="30" align="right" bgcolor="#F7F7F7">
