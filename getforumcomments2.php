<?php
	...
	if( $strcids != ''){
	  ...
	  {  
		 ...
		 $comment_query = mysql_query("SELECT * FROM sas_comment WHERE cid IN ( ".$strcids." ) order by date desc limit $start, $end") or die(mysql_error());
		 
         $query = mysql_query("SELECT * FROM sas_comment WHERE cid IN ( ".$strcids." ) order by date desc ") or die(mysql_error());
...
?> 
		  ...
		  <div class="comment-block" id="comment-blockk">
				...
				?>
					<div class="comment-item">
						...
						<div class="comment-post">
						  <div class="comment-post-div">
						    <div class="comment-post-left" >
							  <h6><?php if(isset($_SESSION['uid']) && $_SESSION['uid'] == $comment['uid']){ echo "Me"; }else if(!empty($rsrslt['username'])){ echo $rsrslt['username'];}else{ echo "Anonymous"; }?> <span style="color:#000;">said....</span></h6>
							</div>  
							<div class="comment-post-right" style="font-size:10px;">	
							<?php
				  ...
				  <?php 
				     ...
				     $usql="select * from sas_users where user_id='$comment[uid]'";
				  ...
				  <div class="comment-item rply-comment-div" id="rply-comment-div<?php echo $comment['id'];?>" style="width: 90%;float: right;background-color: #E6E6E6;" >
					  ...
					  <div class="comment-post">
					     <div class="comment-post-div">
						    <div class="comment-post-left" >
							   <h6><?php if(!empty($ursrslt['username'])){ echo "Me"; }?> <span style="color:#000;">replying...</span></h6>
							</div>							
							<div class="comment-post-right" style="font-size:10px;">
						 ...
						 <div style="width:100%">						     	
                             <div class="" >
							  <form class="rply-form" name="rply-form" id="rply-form" method="post" action="" >
								...
								<input type="submit" style="width: 15%;" class="rply-submit" name="rply-submit" value="Reply"> &nbsp;&nbsp; <input type="button" style="width: 15%;" class="cancel-rply" data-id="<?php echo $comment['id'];?>" value="Cancel">
							  </form>
							 </div>
						 </div>
					  </div></br><br>
			  ...
			  ?>  
				     <div class="comment-item" style="width: 90%;float: right;background-color: #E6E6E6;">
						<div class="comment-avatar">
							<?php 
						...
						<div class="comment-post">
						  <div class="comment-post-div">
						    <div class="comment-post-left" >
							  <h6><?php if( $crrslt['adminid'] != 0 ){ echo 'Admin';}else if( $crrslt['mdrid'] != 0 ){ echo 'Moderator'; } else if( $crrslt['uid'] != 0 ){ if( isset($_SESSION['uid']) && $_SESSION['uid'] == $crrslt['uid'] ){ echo "Me";}else{ echo $arsrslt['username'];} }else if( $crrslt['anuid'] != 0 ){ echo "Anonymous";} ?> <span style="color:#000;">says....</span></h6>
							</div>  
							<div class="comment-post-right" style="font-size:10px;">
                            <!--							
							<?php
			...
			/* ----------------------------------------------------------------------------------------------------------- */
			$msg .= "<div class='pagination'><ul id='pagination'>";

			// FOR ENABLING THE FIRST BUTTON
			if ($first_btn && $cur_page > 1) {
				
				$msg .= "<li p='1' type='$themeids'  class='active'>First</li>";
				
			} else if ($first_btn) {
				$msg .= "<li p='1' type='$themeids'  class='inactive'>First</li>";
			}

			// FOR ENABLING THE PREVIOUS BUTTON
			if ($previous_btn && $cur_page > 1) {
				$pre = $cur_page - 1;
				$msg .= "<li p='$pre' type='$themeids'  class='active'>Previous</li>";
			...
			for ($i = $start_loop; $i <= $end_loop; $i++) {
				...
				if ($cur_page == $i)
					$msg .= "<li p='$i' type='$themeids' style='color:#fff;background-color:#005292;' class='active'>{$i}</li>";
				else
					$msg .= "<li p='$i' type='$themeids'  class='active'>{$i}</li>";
			}

			// TO ENABLE THE NEXT BUTTON
			if ($next_btn && $cur_page < $no_of_paginations) {
				$nex = $cur_page + 1;
				$msg .= "<li p='$nex' type='$themeids'  class='active'>Next</li>";
			} else if ($next_btn) {
			...
			if ($last_btn && $cur_page < $no_of_paginations) {
				$msg .= "<li p='$no_of_paginations' type='$themeids'  class='active'>Last</li>";
			} else if ($last_btn) {
				$msg .= "<li p='$no_of_paginations' type='$themeids'  class='inactive'>Last</li>";
			}
			$goto = "<input type='text' class='goto'  size='1' />
			<input type='button' difftype='$themeids'  id='go_btn' class='go_button' value='Go'/>";
		...
		<script>
		 ...
		 $(document).ready(function(){
			...
			function loadData(page,type){
				...
				$.post('getforumcomments.php',{"page":page,'type':type,'vcid':vcid,'cid':vcid},function(data){
						loading_hide();
						//alert(data)
